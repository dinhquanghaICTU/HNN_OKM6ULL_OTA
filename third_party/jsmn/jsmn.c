...
#ifndef os_trace
#define os_trace(fmt, arg)   do{}while(0)
#endif
                 ...
                 const char *key)
{
    ...
    char data[32];
    memset(data, 0, 32);
    memcpy(data, json + tok[pos].start, tok[pos].end - tok[pos].start);
    return str2int(data);
}
                 ...
                 const char *key, char *buf)
{
    ...
    memcpy(buf, json + tok[pos].start, tok[pos].end - tok[pos].start);
    return strlen(buf);
}
                     ...
                     const char *key, int index, char *buf)
{
    ...
    sprintf(buf, "%.*s", g->end - g->start, json + g->start);
