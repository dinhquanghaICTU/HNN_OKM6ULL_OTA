...
extern uint8_t g_btn_filter_cnt;
...
void app_btn_initialize(app_btn_config_t *conf)
{
    ...
    if (NULL == conf)
    {
        // tlk_printf("ERR: cfg NULL ");
        return;
    }

    if (APP_BTN_MAX_BTN_SUPPORT < conf->btn_count)
    {
        // tlk_printf("ERR: btn support: %d ", conf->btn_count);
        return;
    }
...
void app_btn_scan(void *params)
{
    ...
    if (!m_hw_params)
    {
        // tlk_printf("ERR: hw params NULL ");
        return;
    }

    if (APP_BTN_MAX_BTN_SUPPORT < m_btn_config.btn_count)
    {
        // tlk_printf("ERR: btn support: %d ", m_btn_config.btn_count);
        return;
    }
    ...
    for (i = 0; i < m_btn_config.btn_count; i++)
    {
        ...
        if (m_btn_data[i] != m_hw_params[i].idle_level)
        {
            if (now - m_btn_hold_timestamp[i] >= app_btn_on_hold_time_fire_event_ms)
            {
                ...
                if (((m_btn_hold_evt >> i) & 0x1)) /* hold before*/
                {
        ...
        if ((m_btn_press_evt >> i) & 0x01)
        {
            ...
            {
                tlk_printf(DEBUG_TOUCHPAD,"ERR: cb for press NULL ");
            }
            m_btn_press_evt &= ~(1 << i);
        }
        if ((m_btn_release_evt >> i) & 0x01)
        {
            ...
            {
                tlk_printf(DEBUG_TOUCHPAD,"ERR: cb for release NULL ");
            }
            m_btn_release_evt &= ~(1 << i);
        ...
        if ((m_btn_on_hold_evt >> i) & 0x1)
        {
            ...
            {
                m_callback_table[APP_BTN_EVT_HOLD_LONG](m_hw_params[i].pin,
                                                        APP_BTN_EVT_HOLD_LONG,
                                                        (void *)&hold_time);
                // blog_warn("send evt hold long ");
            }
            m_btn_on_hold_evt &= ~(1 << i);
        }
    }
